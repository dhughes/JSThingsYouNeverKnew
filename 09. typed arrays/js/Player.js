...
Player.identifyMoves = function(gameState, collection){
	...
	for(f = 0 ; f < piles.tableaux.length ; f++){
		...
		if(Card.numericValue(card) === lowestFoundationValue + 1){
			collection.add({stateScore: gameState.stateScore, moveScore: 6 + bonus, from: "tableaux", fromIndex: f, to: "foundations", toIndex: foundationSuits.indexOf(Card.numericSuit(card)), hash: gameState.hash});
			identifiedMoves++;
	...
	if(pileLengths.waste){
		...
		if(Card.numericValue(card) === lowestFoundationValue + 1){
			collection.add({stateScore: gameState.stateScore, moveScore: 6, from: "waste", to: "foundations", toIndex: foundationSuits.indexOf(Card.numericSuit(card)), hash: gameState.hash});
			identifiedMoves++;
			return; // note, once we find one card that needs to go to the foundation, do no more.  If there are more, we'll find them in the next test.
		}
	...
	for(f = 0 ; f < piles.tableaux.length ; f++){
		...
		if(Card.numericValue(to[toLen-1]) === Card.numericValue(card) - 1){
			collection.add({stateScore: gameState.stateScore, moveScore: 5 + bonus, from: "tableaux", fromIndex: f, to: "foundations", toIndex: t, hash: gameState.hash});
			identifiedMoves++;
		}
	...
	if(pileLengths.waste){
		...
		if(toLen && Card.numericSuit(to[toLen-1]) === cardNumericSuit && Card.numericValue(to[toLen-1]) === Card.numericValue(card) - 1){
			collection.add({stateScore: gameState.stateScore, moveScore: 5, from: "waste", to: "foundations", toIndex: t, hash: gameState.hash});
			identifiedMoves++;
		}
	}
		...
		for(var c = fromLen-1 ; c >= 0 && Card.isFaceUp(from[c]) ; c--){
			...
			if(validToMove){
				...
				for(t = 0 ; t < 7 ; t++){
					...
					if(toLen && Card.color(to[toLen-1]) !== cardColor && Card.numericValue(to[toLen-1]) === cardNumericValue + 1){
						// by moving this card, do we expose another new card?

						collection.add({stateScore: gameState.stateScore, moveScore: 3 + bonus, from: "tableaux", fromIndex: f, count: fromLen - c, to: "tableaux", toIndex: t, hash: gameState.hash});
						identifiedMoves++;

					} else if(!toLen && cardNumericValue === 13){
						// if the to tableau is empty and we have a king we can move it here

						collection.add({stateScore: gameState.stateScore, moveScore: 3 + bonus, from: "tableaux", fromIndex: f, to: "tableaux", toIndex: t, hash: gameState.hash});
						identifiedMoves++;
						break;
					}
				}
	...
	if(pileLengths.waste){
		...
		for(t = 0 ; t < 7 ; t++){
			...
			if(toLen && Card.color(to[toLen-1]) !== cardColor && Card.numericValue(to[toLen-1]) === cardNumericValue + 1){
				...
				collection.add({stateScore: gameState.stateScore, moveScore: 3, from: "waste", to: "tableaux", toIndex: t, hash: gameState.hash});
				identifiedMoves++;
			} else if(!toLen && cardNumericValue === 13){
				// if the to tableau is empty and we have a king, we can move it here
				//todo: test in unit test
	...
	if(pileLengths.stock){
		collection.add({stateScore: gameState.stateScore, moveScore: 0, from: "stock", to: "waste", count: gameState.drawCount, hash: gameState.hash});
		identifiedMoves++;
	} else if(pileLengths.waste){
		//todo: test in unit test
		collection.add({stateScore: gameState.stateScore, moveScore: 0, from: "waste", to: "stock", count: pileLengths.waste, hash: gameState.hash});
		identifiedMoves++;
	}
	...
	for(f = 0 ; f < piles.foundations.length ; f++){
		...
		for(t = 0 ; t < piles.tableaux.length ; t++){
			...
			if(toLen && Card.color(to[toLen-1]) !== cardColor && Card.numericValue(to[toLen-1]) === cardNumericValue + 1){
				...
				collection.add({stateScore: gameState.stateScore, moveScore: 0, from: "foundations", fromIndex: f, to: "tableaux", toIndex: t, hash: gameState.hash});
				identifiedMoves++;
			} else if(!toLen && cardNumericValue === 13){
				// if the to tableau is empty and we have a king, we can move it here
